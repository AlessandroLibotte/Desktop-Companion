...
__version__ = '1.0.1 underAlex Edit'
...
class SystemTray:
    ...
    def show_message(self, title=None, message=None):
        ...
        :type message: str
        """
        self.tray_icon.notify(title=str(title) if title is not None else '',
    ...
    # ------------ underAlex Edit -----------
    def _on_clicked(self, icon, item: pystray.MenuItem, key=None, ):
        self.window.write_event_value(key, item.text)

    # ------------ underAlex Edit -----------

    def _convert_psg_menu_to_tray(self, psg_menu, sub_index=None):
        menu_items = []
        i = 0
            ...
            while i < len(psg_menu):
                ...
                if not isinstance(item, list) and not isinstance(look_ahead, list):
                    ...
                    if sub_index is not None:
                        self.submenu_keys.append(self.submenu_keys[sub_index] + "-" + item)
                    # ------------ underAlex Edit -----------
                    disabled = False
                    ...
                    if not (item == pystray.Menu.SEPARATOR and sg.running_linux()):
                        ...
                        if sub_index is not None:
                            def make_lambda(x):
                                return lambda _, item: self._on_clicked(_, item=item, key=x)

                            menu_items.append(
                                pystray.MenuItem(item, make_lambda(self.submenu_keys[-1]), enabled=not disabled,
                                                 default=False))
                        else:
                            def make_lambda():
                                return lambda _, item: self._on_clicked(_, item=item)

                            menu_items.append(
                                pystray.MenuItem(item, make_lambda(), enabled=not disabled, default=False))
                        # ------------ underAlex Edit -----------
                elif look_ahead != item:
                    if isinstance(look_ahead, list):
                        if menu_items is None:
                            menu_items = pystray.MenuItem(item,
                                                          pystray.Menu(*self._convert_psg_menu_to_tray(look_ahead)))
                        else:
                            # ------------ underAlex Edit -----------
                            if sub_index is not None:
                                self.submenu_keys.append(self.submenu_keys[sub_index] + "-" + item)
                                index = self.submenu_keys.index(self.submenu_keys[-1])
                                menu_items.append(pystray.MenuItem(item, pystray.Menu(
                                    *self._convert_psg_menu_to_tray(look_ahead, index))))
                            else:
                                self.submenu_keys.append(item)
                                index = self.submenu_keys.index(self.submenu_keys[-1])
                                menu_items.append(pystray.MenuItem(item, pystray.Menu(
                                    *self._convert_psg_menu_to_tray(look_ahead, index))))
                            # ------------ underAlex Edit -----------
        ...
        menu_items.append(
            pystray.MenuItem('default', self._default_action_callback, enabled=True, default=True, visible=False))
...
def main():
    ...
    layout = [[sg.Text('My PySimpleGUI Window with a Tray Icon - X will minimize to tray')],
              ...
              [sg.Button('Go'), sg.B('Hide Icon'), sg.B('Show Icon'), sg.B('Hide Window'), sg.B('Close Tray'),
               sg.Button('Exit')]]
